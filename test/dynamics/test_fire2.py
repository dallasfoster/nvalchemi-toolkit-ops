...
def make_fire2_state(
    N, M, dtype_vec, dtype_scalar, np_dtype, device, *, rng=None
):
    ...
    return (
        ...
        alpha, dt, nsteps_inc,
        pos_np.copy(), vel_np.copy(), forces_np.copy(), batch_idx_np.copy(),
        alpha_np.copy(), dt_np.copy(), nsteps_inc_np.copy(),
    )
...
def make_fire2_torch_state(N, M, torch_dtype, device, *, rng=None):
    ...
    return (
        pos, vel, forces, batch_idx, alpha, dt, nsteps_inc,
        pos_np.copy(), vel_np.copy(), forces_np.copy(), bidx_np.copy(),
        alpha_np.copy(), dt_np.copy(), nsteps_np.copy(),
    )
...
class TestFire2Step:
    ...
    def test_single_system(self, device, dtype_vec, dtype_scalar, np_dtype):
        ...
        (
            pos, vel, forces, bidx,
            alpha, dt, nsteps_inc,
            pos_np, vel_np, forces_np, bidx_np,
            alpha_np, dt_np, nsteps_np,
        ) = make_fire2_state(N, M, dtype_vec, dtype_scalar, np_dtype, device)

        vf, v_sumsq, f_sumsq, max_norm = make_fire2_scratch(
            M, dtype_scalar, device
        )
...
class TestFire2Convergence:
    ...
    def test_anharmonic_convergence(self, device):
        ...
        pos_np = np.array(
            [[0.0, 0.0, 0.0], [2.5, 0.0, 0.0]], dtype=np_dtype
        )
        vel_np = np.zeros((N, 3), dtype=np_dtype)
        bidx_np = np.zeros(N, dtype=np.int32)
        alpha_np = np.full(1, 0.09, dtype=np_dtype)
...
class TestFire2TorchCoord:
    ...
    def test_coord_step_correctness(self, device, torch_dtype):
        ...
        np.testing.assert_allclose(
            pos.cpu().numpy(), pos_np, rtol=rtol, atol=1e-7,
        )
        np.testing.assert_allclose(
            vel.cpu().numpy(), vel_np, rtol=rtol, atol=1e-7,
        )
        np.testing.assert_allclose(
            alpha.cpu().numpy(), alpha_np, rtol=rtol,
        )
        np.testing.assert_allclose(
            dt.cpu().numpy(), dt_np, rtol=rtol,
        )
        np.testing.assert_array_equal(
            nsteps_inc.cpu().numpy(), nsteps_np,
        )
    ...
    def test_coord_step_scratch_reuse(self, device):
        ...
        torch.cuda.synchronize()
        np.testing.assert_allclose(
            pos.cpu().numpy(), pos_np, rtol=1e-8, atol=1e-7,
        )
        np.testing.assert_allclose(
            vel.cpu().numpy(), vel_np, rtol=1e-8, atol=1e-7,
        )
...
class TestFire2AlgorithmicBehavior:
    ...
    def test_uphill_resets_state(self, device):
        ...
        _fire2_reference_step(
            pos_ref, vel_ref, forces_ref, bidx_ref,
            alpha_ref, dt_ref, nsteps_ref,
            **FIRE2_DEFAULTS,
        )
    ...
    def test_acceleration_after_delaystep(self, device):
        ...
        np.testing.assert_allclose(pos.numpy(), pos_ref, rtol=1e-10, atol=1e-12)
        np.testing.assert_allclose(
            alpha.numpy(), alpha_ref, rtol=1e-10, atol=1e-12,
        )
        np.testing.assert_allclose(dt_arr.numpy(), dt_ref, rtol=1e-10)
...
class TestFire2EdgeCases:
    ...
    def test_single_atom(self, device):
        ...
        (
            ...
            N, M, dtype_vec, dtype_scalar, np_dtype, device, rng=rng
        )
    ...
    def test_zero_forces_nonzero_velocities(self, device):
        ...
        np.testing.assert_allclose(
            pos.numpy(), pos_ref, rtol=1e-10, atol=1e-12,
        )
    ...
    def test_zero_initial_velocities(self, device):
        ...
        np.testing.assert_allclose(
            final_pos[:, 1:], pos_before[:, 1:], atol=1e-12,
...
class TestFire2TorchCoordErrors:
    ...
    def test_none_batch_idx(self, device):
        ...
        (
            pos, vel, forces, _bidx, alpha, dt, nsteps_inc,
            *_,
        ) = make_fire2_torch_state(N, M, torch_dtype, device)

        with pytest.raises((AttributeError, TypeError)):
            fire2_step_coord(
                pos, vel, forces, None, alpha, dt, nsteps_inc,
                **FIRE2_DEFAULTS,
            )
    ...
    def test_unsupported_dtype(self, device):
        ...
        with pytest.raises(KeyError):
            fire2_step_coord(
                pos_f16, vel, forces, batch_idx, alpha, dt, nsteps_inc,
                **FIRE2_DEFAULTS,
            )
