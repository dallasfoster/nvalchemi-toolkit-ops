...
r"""Multipole PME binding parity tests.

Covers ``multipole_pme_green_structure_factor``, ``multipole_pme_gather_*``,
``multipole_pme_reciprocal_space``, and ``multipole_particle_mesh_ewald``
(single + batched). Spread coverage is end-to-end through the
reciprocal-space and particle-mesh-Ewald composites.
"""
...
# Trampoline kernel — evaluates B(theta, offset) at a Python-supplied
# (atom, point_idx) tuple to build the spline oracle without re-implementing
# the spline math in numpy.
