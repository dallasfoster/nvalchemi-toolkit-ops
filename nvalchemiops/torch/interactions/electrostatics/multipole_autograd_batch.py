...
@torch.library.custom_op(
    "nvalchemiops::batch_multipole_rho_phihat_grad",
    mutates_args=(),
    schema=(
        "(Tensor charges, Tensor dipoles, Tensor cosines, Tensor sines, "
        "Tensor grad_rho, Tensor volume, Tensor batch_idx) -> Tensor"
    ),
)
def _batch_rho_phihat_grad_op(
    charges: torch.Tensor,
    dipoles: torch.Tensor,
    cosines: torch.Tensor,
    sines: torch.Tensor,
    ...
    """``dL/dsource_phi_hat`` ``(B, K_max, 4, 2)`` via ``batch_rho_phihat_grad``."""
    device = charges.device
    wp_device = wp.device_from_torch(device)
    wp_scalar = _wp_scalar(charges.dtype)
    vec_dtype = _wp_vec(charges.dtype)
...
@torch.library.register_fake("nvalchemiops::batch_multipole_rho_phihat_grad")
def _batch_rho_phihat_grad_fake(
    charges: torch.Tensor,
    dipoles: torch.Tensor,
    cosines: torch.Tensor,
    sines: torch.Tensor,
...
@torch.library.custom_op(
    "nvalchemiops::batch_multipole_rho_kphase_grad",
    mutates_args=(),
    schema=(
...
def _batch_rho_kphase_grad_op(
    ...
    """``dL/dk_vectors`` ``(B, K_max, 3)`` through the phase via ``batch_rho_kphase_grad``."""
    device = charges.device
    wp_device = wp.device_from_torch(device)
    wp_scalar = _wp_scalar(charges.dtype)
    vec_dtype = _wp_vec(charges.dtype)
...
@torch.library.register_fake("nvalchemiops::batch_multipole_rho_kphase_grad")
def _batch_rho_kphase_grad_fake(
    charges: torch.Tensor,
    dipoles: torch.Tensor,
    positions: torch.Tensor,
    cosines: torch.Tensor,
...
@torch.library.custom_op(
    "nvalchemiops::batch_multipole_rho_q_coeff2_grad",
    mutates_args=(),
    schema=(
...
def _batch_rho_q_coeff2_grad_op(
    quadrupoles: torch.Tensor,
    cosines: torch.Tensor,
    sines: torch.Tensor,
    k_vectors: torch.Tensor,
    ...
    """``dL/dsource_coeff2`` ``(B, K_max)`` via ``batch_rho_q_coeff2_grad`` (l=2 stress)."""
    device = quadrupoles.device
    wp_device = wp.device_from_torch(device)
    wp_scalar = _wp_scalar(quadrupoles.dtype)
    mat_dtype = wp.mat33d if wp_scalar == wp.float64 else wp.mat33f
...
@torch.library.register_fake("nvalchemiops::batch_multipole_rho_q_coeff2_grad")
def _batch_rho_q_coeff2_grad_fake(
    quadrupoles: torch.Tensor,
    cosines: torch.Tensor,
    sines: torch.Tensor,
    k_vectors: torch.Tensor,
...
@torch.library.custom_op(
    "nvalchemiops::batch_multipole_rho_q_kvec_grad",
    mutates_args=(),
    schema=(
...
def _batch_rho_q_kvec_grad_op(
    ...
    """``dL/dk_vectors`` ``(B, K_max, 3)`` via the (k.Q.k) form + phase (l=2 stress)."""
    device = quadrupoles.device
    wp_device = wp.device_from_torch(device)
    wp_scalar = _wp_scalar(quadrupoles.dtype)
    mat_dtype = wp.mat33d if wp_scalar == wp.float64 else wp.mat33f
...
@torch.library.register_fake("nvalchemiops::batch_multipole_rho_q_kvec_grad")
def _batch_rho_q_kvec_grad_fake(
    quadrupoles: torch.Tensor,
    positions: torch.Tensor,
    cosines: torch.Tensor,
    sines: torch.Tensor,
